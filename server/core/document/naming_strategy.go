...
func NewNamingStrategyDDDYY() *RegexStrategy {
	return &RegexStrategy{
...
func NewNamingStrategyPrefixDDDYY(prefix string) *RegexStrategy {
	return &RegexStrategy{
...
// NewNamingStrategyDDDYYWithYear is a deterministic variant useful for tests.
func NewNamingStrategyDDDYYWithYear(year int) *RegexStrategy {
	return &RegexStrategy{
		Pattern: regexp.MustCompile(`^([0-9]{3}-[0-9]{2})$`),
		Format:  "%s",
		CodeFmt: &YearSuffixDecimalFormat{
			Padding:      3,
			YearProvider: func() int { return year },
		},
	}
}

// NewNamingStrategyPrefixDDDYYWithYear is a deterministic variant useful for tests.
func NewNamingStrategyPrefixDDDYYWithYear(prefix string, year int) *RegexStrategy {
	return &RegexStrategy{
		Pattern: regexp.MustCompile(fmt.Sprintf(`^%s-([0-9]{3}-[0-9]{2})$`, prefix)),
		Format:  prefix + "-%s",
		CodeFmt: &YearSuffixDecimalFormat{
