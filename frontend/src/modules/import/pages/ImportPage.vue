<template>
  <div class="space-y-4">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
      <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Import Documents</h1>
    </div>
      ...
      <div class="space-y-6">
        ...
        <details v-if="selectedFile" class="group" open>
          ...
          <div class="pt-4 mt-2 border-t border-gray-100 space-y-8">
            <div>
              <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Core Document Fields</h3>
            ...
            <div v-if="customFieldKeys.length > 0">
              ...
              <p class="text-[11px] text-gray-500 mb-4 italic">These fields are unmapped by default. Select a column to include them.</p>
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
...
const coreFieldKeys = ['code', 'title', 'nature', 'status', 'complaint', 'complainants', 'respondents', 'created_at'];
...
const initializeMapping = () => {
  ...
  detectedHeaders.value.forEach(header => {
    ...
    const coreMatch = coreFieldKeys.find(key => 
      key === cleanHeader || cleanHeader.replace(/\s/g, '_') === key
    );

    if (coreMatch) {
      columnMap[coreMatch] = header;
    } else {
      const customKey = cleanHeader.replace(/\s+/g, '_');
      columnMap[customKey] = '';
    }
  });
};
...
const convertToISODate = (dateStr: string): string => {
  ...
  return !isNaN(timestamp) ? new Date(timestamp).toISOString() : dateStr;
};

const mapRowToDocument = (row: Record<string, string>) => {
  ...
  const doc: any = {
    code: getVal('code'),
    title: getVal('title'),
    created_at: convertToISODate(getVal('created_at')),
    fields: {
      nature: getVal('nature').toLowerCase(),
      status: getVal('status').toLowerCase().replace(/\s+/g, '_'),
      complaint: getVal('complaint'),
      complainants: getList('complainants'),
      respondents: getList('respondents'),
    }
  };

  customFieldKeys.value.forEach(key => {
    const val = getVal(key);
    if (val) doc.fields[key] = val;
  });
    ...
    try {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        try {
          ...
          const payload = mapRowToDocument(row);
          if (!payload.title) {
            errorDetails.value.push(`Row ${rowNum}: Title column is unmapped.`);
            continue; 
          }
      ...
      importResult.value = {
        ...
        message: `Import finished. Successfully created ${successCount} of ${dataRows.length} documents.`,
