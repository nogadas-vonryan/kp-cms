<template>
  <div class="space-y-4">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        <button 
          ...
          :class="calendarStore.isLoading ? 'bg-gray-100 text-gray-500' : (calendarStore.isConnected ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200')"
          :disabled="calendarStore.isLoading"
        >
          <svg v-if="calendarStore.isLoading" class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <svg v-else-if="calendarStore.isConnected" class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
          </svg>
          <svg v-else class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
          </svg>
          <span>{{ calendarStore.isLoading ? 'Loading...' : (calendarStore.isConnected ? 'Calendar Connected' : 'Connect Calendar') }}</span>
        ...
        <button 
          ...
          @click="calendarStore.refreshAll()"
          class="p-1.5 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors"
          title="Refresh all"
        >
          <RefreshCw class="h-4 w-4" :class="{ 'animate-spin': calendarStore.isLoading || calendarStore.calendarsLoading || calendarStore.eventsLoading }" />
        </button>
      </div>
    </div>
    ...
    <UiModal v-model:open="showSettingsModal" title="Calendar Settings">
      <div class="space-y-6">
        <OAuthConnectionCard @connected="calendarStore.refreshAll()" @disconnected="calendarStore.clearConnection()" />
