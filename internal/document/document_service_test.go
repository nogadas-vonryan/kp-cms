...
type mockRepo struct {
	DocumentRepository
	OnGetByUUID   func(ctx context.Context, uuid string) (*DocumentResponse, error)
	OnGetByCode   func(ctx context.Context, code string) (*DocumentResponse, error)
	OnGetByFolder func(ctx context.Context, folder string) (*DocumentResponse, error)
	OnCreate      func(ctx context.Context, doc *Document) error
	OnUpdate      func(ctx context.Context, uuid string, doc *Document) error
	OnDelete      func(ctx context.Context, uuid string) error
	OnList        func(ctx context.Context) ([]*Document, error)
}

func (m *mockRepo) GetByUUID(ctx context.Context, uuid string) (*DocumentResponse, error) {
	if m.OnGetByUUID != nil {
		return m.OnGetByUUID(ctx, uuid)
	}
	return nil, nil
}
func (m *mockRepo) GetByCode(ctx context.Context, code string) (*DocumentResponse, error) {
	if m.OnGetByCode != nil {
		return m.OnGetByCode(ctx, code)
	}
	return nil, nil
}
func (m *mockRepo) GetByFolderName(ctx context.Context, folder string) (*DocumentResponse, error) {
	if m.OnGetByFolder != nil {
		return m.OnGetByFolder(ctx, folder)
	}
	return nil, nil
}
func (m *mockRepo) Create(ctx context.Context, doc *Document) error {
	if m.OnCreate != nil {
		return m.OnCreate(ctx, doc)
	}
	return nil
}
func (m *mockRepo) Update(ctx context.Context, uuid string, doc *Document) error {
	if m.OnUpdate != nil {
...
func (m *mockRepo) List(ctx context.Context) ([]*Document, error) {
	if m.OnList != nil {
		return m.OnList(ctx)
	}
	return nil, nil
...
func TestDocumentService_Create(t *testing.T) {
	...
	t.Run("should fail if code is empty", func(t *testing.T) {
		service := NewDocumentService(nil)
		doc := Document{Title: "Test Title", Code: "", FolderName: "case_0001"}

		err := service.Create(ctx, doc)
		if err == nil || err.Error() != "document code is required" {
			t.Errorf("expected title error, got: %v", err)
		}
	})
	...
	t.Run("should fail if folder name is empty", func(t *testing.T) {
		service := NewDocumentService(nil)
		doc := Document{Title: "Test Title", Code: "0001", FolderName: ""}

		err := service.Create(ctx, doc)
		if err == nil || err.Error() != "document folder name is required" {
			t.Errorf("expected title error, got: %v", err)
		}
	})

	t.Run("should fail if document with same folder name exists", func(t *testing.T) {
		mockRepo := &mockRepo{
			OnGetByFolder: func(ctx context.Context, folder string) (*DocumentResponse, error) {
				return &DocumentResponse{UUID: "123", Code: "0001", FolderName: folder}, nil
			},
		}
		service := NewDocumentService(mockRepo)
		doc := Document{Title: "Test Title", Code: "0002", FolderName: "case_0001"}

		err := service.Create(ctx, doc)
		if err == nil || err.Error() != "document by that folder name already exists" {
			t.Errorf("expected folder name exists error, got: %v", err)
		}
	})

	t.Run("should fail if document with same code exists", func(t *testing.T) {
		mockRepo := &mockRepo{
			OnGetByFolder: func(ctx context.Context, folder string) (*DocumentResponse, error) {
				return nil, nil
			},
			OnGetByCode: func(ctx context.Context, code string) (*DocumentResponse, error) {
				return &DocumentResponse{UUID: "123", Code: code, FolderName: "case_0001"}, nil
			},
		}
		service := NewDocumentService(mockRepo)
		doc := Document{Title: "Test Title", Code: "0001", FolderName: "case_0002"}

		err := service.Create(ctx, doc)
		if err == nil || err.Error() != "document by that code already exists" {
			t.Errorf("expected code exists error, got: %v", err)
		}
	})
...
func TestDocumentService_GetByUUID(t *testing.T) {
	...
	t.Run("should return document by uuid", func(t *testing.T) {
		expectedDoc := &DocumentResponse{UUID: "test-uuid", Code: "0001", Title: "Test"}
		mockRepo := &mockRepo{
			OnGetByUUID: func(ctx context.Context, uuid string) (*DocumentResponse, error) {
...
func TestDocumentService_GetByFolderName(t *testing.T) {
	...
	t.Run("should return document by folder name", func(t *testing.T) {
		expectedDoc := &DocumentResponse{UUID: "test-uuid", Code: "0001", FolderName: "case_0001"}
		mockRepo := &mockRepo{
			OnGetByFolder: func(ctx context.Context, folder string) (*DocumentResponse, error) {
				return expectedDoc, nil
			},
...
func TestDocumentService_Update(t *testing.T) {
	...
	t.Run("should fail if uuid is empty", func(t *testing.T) {
		...
		err := service.Update(ctx, "", Document{})
		if err == nil || err.Error() != "uuid is required" {
			t.Errorf("expected uuid required error, got: %v", err)
		}
	})
	...
	t.Run("should fail if title is empty", func(t *testing.T) {
		...
		err := service.Update(ctx, "test-uuid", Document{Title: ""})
		if err == nil || err.Error() != "document title is required" {
	...
	t.Run("should update document successfully", func(t *testing.T) {
		...
		err := service.Update(ctx, "test-uuid", Document{Title: "Updated Title"})
		if err != nil {
			t.Errorf("expected no error, got: %v", err)
		}
	})
